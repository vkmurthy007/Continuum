...
export default function SignalFeed({ records }: Props) {
  if (records.length === 0) {
    return (
      <div className="p-8 text-center text-gray-600 text-sm">No signals for selected population.</div>
    );
  }

  return (
    <div className="p-4 space-y-3">
      <div className="text-xs text-gray-500 uppercase tracking-wider mb-2">Live Signal Feed</div>
      {records.map(rec => (
        <div key={rec.record_id} className="bg-gray-900 border border-gray-800 rounded-xl p-4 font-mono text-xs">
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-2">
              <span className="text-gray-600">signal</span>
              <span className={`px-2 py-0.5 rounded border text-xs font-bold ${severityColor[rec.severity]}`}>
                {rec.severity.toUpperCase()}
              </span>
            </div>
            <span className="text-gray-700">{rec.condition}</span>
          </div>

          <pre className="text-gray-300 text-xs leading-relaxed overflow-x-auto whitespace-pre-wrap">{JSON.stringify({
          ...
          <div className="mt-3 pt-3 border-t border-gray-800 flex items-center gap-2 flex-wrap">
            {rec.export_destinations.map(dest => (
              <span key={dest} className="px-2 py-1 bg-gray-800 text-gray-400 rounded text-xs">{dest}</span>
            ))}
          </div>
        </div>
      ))}
